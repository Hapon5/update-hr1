...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($email) || empty($password)) {
        ...
        if ($user && strtolower($email) === 'employee@gmail.com' && $user['Account_type'] != 3 && $passwordMatches) {
...
<body class="bg-gray-100 flex items-center justify-center h-screen" style="background-image: url('../Image/crane.jpg'); background-size: cover; background-position: center;">
    <div class="bg-white p-8 rounded-lg shadow-xl w-full max-w-md relative overflow-hidden">
        <div class="absolute top-0 left-0 w-full h-2 bg-blue-600"></div>
        
        <div class="text-center mb-8">
            <h2 class="text-3xl font-bold text-gray-800">Employee Portal</h2>
        ...
        <form method="POST" class="space-y-6">
            <div>
                ...
                <input type="email" name="email" class="w-full px-4 py-3 rounded-lg bg-gray-50 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all" required autofocus placeholder="employee@company.com">
            </div>
            <div>
                <label class="block text-gray-700 text-sm font-bold mb-2">Password</label>
                <input type="password" name="password" class="w-full px-4 py-3 rounded-lg bg-gray-50 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all" required placeholder="••••••••">
            </div>
            <button type="submit" class="w-full bg-blue-600 text-white font-bold py-3 rounded-lg hover:bg-blue-700 transition-colors shadow-lg">
                Sign In
            </button>
        </form>
